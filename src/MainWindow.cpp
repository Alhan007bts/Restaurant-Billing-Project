...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), currentTableNumber(1) {
  for (int i = 1; i <= 8; ++i) {
    manager.addTable(i);
  }
  setupUI();
...
void MainWindow::setupUI() {
  ...
  quantitySpinner = new QSpinBox(centralWidget);
  quantitySpinner->setRange(1, 100);
  quantitySpinner->setValue(1);
  quantitySpinner->setAlignment(Qt::AlignCenter);
  quantitySpinner->setFixedWidth(60);
  addControlLayout->addWidget(quantitySpinner);

  QVBoxLayout *spinnerBtnLayout = new QVBoxLayout();
  spinnerBtnLayout->setSpacing(1);
  spinnerBtnLayout->setContentsMargins(0, 0, 0, 0);

  QPushButton *upButton = new QPushButton("▲", centralWidget);
  upButton->setObjectName("spinnerBtn");
  upButton->setFixedSize(18, 14);
  connect(upButton, &QPushButton::clicked, this, &MainWindow::onPlusClicked);
  spinnerBtnLayout->addWidget(upButton);

  QPushButton *downButton = new QPushButton("▼", centralWidget);
  downButton->setObjectName("spinnerBtn");
  downButton->setFixedSize(18, 14);
  connect(downButton, &QPushButton::clicked, this, &MainWindow::onMinusClicked);
  spinnerBtnLayout->addWidget(downButton);

  addControlLayout->addLayout(spinnerBtnLayout);
...
void MainWindow::onAddItemClicked() {
  ...
  if (selectedRow < 0) {
    QMessageBox::warning(this, "No Selection",
                         "Please select a menu item from the list.");
    return;
  }
...
void MainWindow::onCalculateBillClicked() {
  ...
  ss << "Date/Time: "
     ...
     << "\n";
  ss << "----------------------------------------\n";
  ss << std::left << std::setw(20) << "Item Name" << std::setw(8) << "Price"
     << std::setw(5) << "Qty"
     << "Subtotal\n";
  ss << "----------------------------------------\n";
  ...
  for (const auto &line : items) {
    ...
    ss << std::left << std::setw(20)
       << (item->getName().length() > 18 ? item->getName().substr(0, 17) + "..."
                                         : item->getName())
       << "$" << std::setw(7) << std::fixed << std::setprecision(2) << price
       << std::setw(5) << qty << "$" << itemSubtotal << "\n";
  ...
  if (discountAmount > 0) {
    ss << std::left << std::setw(30)
       << "Discount (" + std::to_string((int)discountSpinner->value()) + "%):"
       << "-$" << discountAmount << "\n";
  }
  if (totalPrepFee > 0) {
    ss << std::left << std::setw(30) << "Preparation Fees:" << "$"
       << totalPrepFee << "\n";
  }
  ss << std::left << std::setw(30)
     << "Tax (" + std::to_string((int)taxSpinner->value()) + "%):" << "$"
     << taxAmount << "\n";
  ss << "========================================\n";
...
void MainWindow::onImportReceiptTriggered() {
  ...
  if (!file) {
    QMessageBox::critical(this, "Error",
                          "Could not open receipt file for reading.");
    return;
  }
...
void MainWindow::applyTheme(bool dark) {
  if (dark) {
    this->setStyleSheet(
        ...
        // Table styling (12px rounded corners)
        ...
        // Invoice Area (12px rounded corners)
        ...
        // Custom QComboBox (Let OS render dropdown arrow, style right corner boundaries)
        ...
        // Custom QSpinBox (Hide default arrows and style number box)
        ...
        // Custom mini vertical spinner buttons (▲ and ▼)
        ...
        // Labels & Titles (BiteKing card left border accent)
...
void MainWindow::onDeleteMenuItemClicked() {
  ...
  QMessageBox::StandardButton reply =
      QMessageBox::question(this, "Confirm Deletion",
                            QString("Are you sure you want to permanently "
                                    "delete '%1' (ID: %2) from the menu?")
...
void MainWindow::onMenuItemChanged(QTableWidgetItem *item) {
  ...
  if (col == 3) { // Price column
    ...
    if (ok && newPrice >= 0.0) {
      ...
      QMessageBox::warning(
          this, "Invalid Price",
          "Please enter a valid positive decimal number for the price.");
    }
