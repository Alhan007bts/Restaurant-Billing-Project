...
MainWindow::MainWindow(QWidget *parent)
  ...
  std::string err;
  DataManager::loadData("data", dineInOrders, restaurantBills,
                        restaurantPayments, &err);
...
void MainWindow::closeEvent(QCloseEvent *event) {
  if (unsavedChanges) {
    QMessageBox::StandardButton reply = QMessageBox::question(
        ...
        "You have unsaved transaction data. Would you like to save before "
        "exiting?",
    ...
    if (reply == QMessageBox::Save) {
      std::string err;
      QDir().mkpath("data");
      if (DataManager::saveData("data", dineInOrders, restaurantBills,
                                restaurantPayments, &err)) {
...
void MainWindow::setupUI() {
  ...
  connect(importMenuAct, &QAction::triggered, this,
          &MainWindow::onImportMenuTriggered);
  fileMenu->addAction(importMenuAct);

  QAction *exportMenuAct = new QAction("&Export Menu...", this);
  connect(exportMenuAct, &QAction::triggered, this,
          &MainWindow::onExportMenuTriggered);
  fileMenu->addAction(exportMenuAct);

  fileMenu->addSeparator();

  QAction *saveDataAct = new QAction("&Save Transaction Data", this);
  connect(saveDataAct, &QAction::triggered, this,
          &MainWindow::onSaveDataTriggered);
  fileMenu->addAction(saveDataAct);

  QAction *loadDataAct = new QAction("&Load Transaction Data", this);
  connect(loadDataAct, &QAction::triggered, this,
          &MainWindow::onLoadDataTriggered);
  fileMenu->addAction(loadDataAct);

  fileMenu->addSeparator();

  QAction *importReceiptAct = new QAction("Import &Receipt...", this);
  connect(importReceiptAct, &QAction::triggered, this,
          &MainWindow::onImportReceiptTriggered);
  fileMenu->addAction(importReceiptAct);

  QAction *exportReceiptAct = new QAction("Export R&eceipt...", this);
  connect(exportReceiptAct, &QAction::triggered, this,
          &MainWindow::onExportReceiptTriggered);
  ...
  connect(darkThemeAct, &QAction::triggered, this,
          &MainWindow::onDarkThemeTriggered);
  ...
  connect(lightThemeAct, &QAction::triggered, this,
          &MainWindow::onLightThemeTriggered);
  ...
  connect(addMenuItemAct, &QAction::triggered, this,
          &MainWindow::onAddMenuItemClicked);
  editMenu->addAction(addMenuItemAct);

  QAction *deleteMenuItemAct = new QAction("&Delete Selected Menu Item", this);
  connect(deleteMenuItemAct, &QAction::triggered, this,
          &MainWindow::onDeleteMenuItemClicked);
  ...
  QLabel *subtitleLabel =
      new QLabel("Smart Restaurant Billing & Ordering System", centralWidget);
...
void MainWindow::setupMenuTab(QWidget *tab) {
  ...
  menuTable->setHorizontalHeaderLabels(
      {"Item ID", "Name", "Category", "Price", "Extra Details"});
  ...
  connect(categoryFilterCombo,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &MainWindow::onSearchFilterChanged);
}

// 2. Orders Tab: Select table, search menu, select quantity, add to active
// order list
void MainWindow::setupOrdersTab(QWidget *tab) {
  ...
  orderMenuTable->setHorizontalHeaderLabels(
      {"Item ID", "Name", "Category", "Price"});
  orderMenuTable->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  ...
  connect(addButton, &QPushButton::clicked, this,
          &MainWindow::onAddItemClicked);
  qtyLayout->addWidget(addButton);
  leftPane->addLayout(qtyLayout);

  // Connect filter signals for active ordering menu
  connect(orderSearchEdit, &QLineEdit::textChanged, this,
          &MainWindow::onOrderSearchFilterChanged);
  connect(orderCategoryFilterCombo,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &MainWindow::onOrderSearchFilterChanged);
  ...
  connect(removeButton, &QPushButton::clicked, this,
          &MainWindow::onRemoveItemClicked);
...
void MainWindow::setupBillingTab(QWidget *tab) {
  ...
  connect(calculateButton, &QPushButton::clicked, this,
          &MainWindow::onCalculateBillClicked);
  ...
  largeBillWarningLabel->setStyleSheet(
      "color: #ef4444; font-weight: bold; border: 2px solid #ef4444; "
      "border-radius: 8px; padding: 6px; background-color: #fef2f2;");
...
void MainWindow::setupPaymentsTab(QWidget *tab) {
  ...
  connect(processPaymentButton, &QPushButton::clicked, this,
          &MainWindow::onProcessPaymentClicked);
  connect(paymentsBillCombo,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &MainWindow::onPaymentBillOrMethodChanged);
  connect(paymentMethodCombo,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &MainWindow::onPaymentBillOrMethodChanged);
...
void MainWindow::populateMenu() {
  ...
  if (!MenuFileIO::loadMenu("data/menu.txt", menuItems, &err)) {
    ...
    menuItems.push_back(
        std::make_shared<FoodItem>("ITEM-0001", "Margherita Pizza",
                                   "Main Course", 12.99, "Italian", 12, false));
    menuItems.push_back(
        std::make_shared<FoodItem>("ITEM-0002", "Spicy Chicken Burger",
                                   "Main Course", 8.99, "American", 8, true));
    menuItems.push_back(std::make_shared<FoodItem>(
        "ITEM-0003", "Bruschetta", "Appetizer", 6.50, "Italian", 5, false));
    menuItems.push_back(
        std::make_shared<FoodItem>("ITEM-0004", "Chocolate Lava Cake",
                                   "Dessert", 7.25, "French", 10, false));
    menuItems.push_back(std::make_shared<BeverageItem>(
        "ITEM-0005", "Iced Latte", "Beverage", 4.50, 350.0, "Coffee", false));
    menuItems.push_back(std::make_shared<BeverageItem>(
        "ITEM-0006", "Coca Cola", "Beverage", 2.50, 330.0, "Soft Drink", true));
    menuItems.push_back(std::make_shared<BeverageItem>(
        "ITEM-0007", "Fresh Orange Juice", "Beverage", 5.00, 400.0, "Juice",
        false));
...
void MainWindow::refreshMenuTable() {
  disconnect(menuTable, &QTableWidget::itemChanged, this,
  ...
  for (size_t i = 0; i < menuItems.size(); ++i) {
    ...
    QTableWidgetItem *idItem =
        new QTableWidgetItem(QString::fromStdString(item->getItemID()));
    ...
    QTableWidgetItem *catItem =
        new QTableWidgetItem(QString::fromStdString(item->getCategory()));
    catItem->setFlags(catItem->flags() & ~Qt::ItemIsEditable);
    menuTable->setItem(i, 2, catItem);

    QTableWidgetItem *priceItem =
        new QTableWidgetItem(QString("$%1").arg(item->getPrice(), 0, 'f', 2));
    priceItem->setFlags(priceItem->flags() | Qt::ItemIsEditable);
    menuTable->setItem(i, 3, priceItem);

    QString details = "";
    if (auto food = std::dynamic_pointer_cast<FoodItem>(item)) {
      details = QString("Cuisine: %1 | Prep: %2 min")
                    .arg(QString::fromStdString(food->getCuisineType()))
                    .arg(food->getPreparationTime());
      if (food->getIsSpicy())
        details += " | SPICY";
    } else if (auto bev = std::dynamic_pointer_cast<BeverageItem>(item)) {
      details = QString("Vol: %1 ml | Type: %2")
                    .arg(bev->getVolume())
                    .arg(QString::fromStdString(bev->getBeverageType()));
      if (bev->getIsCarbonated())
        details += " | CARB";
    }
  ...
  for (size_t i = 0; i < menuItems.size(); ++i) {
    ...
    priceItem->setFlags(
        priceItem->flags() &
        ~Qt::ItemIsEditable); // Non-editable inside the ordering tab
    orderMenuTable->setItem(i, 3, priceItem);
  }

  connect(menuTable, &QTableWidget::itemChanged, this,
          &MainWindow::onMenuItemChanged);
}
...
void MainWindow::onSearchFilterChanged() {
  ...
  for (int r = 0; r < menuTable->rowCount(); ++r) {
    bool matchesName =
        menuTable->item(r, 1)->text().toLower().contains(filterText);
    bool matchesCategory = filterCategory.isEmpty() ||
                           (menuTable->item(r, 2)->text() == filterCategory);
...
void MainWindow::onOrderSearchFilterChanged() {
  ...
  for (int r = 0; r < orderMenuTable->rowCount(); ++r) {
    bool matchesName =
        orderMenuTable->item(r, 1)->text().toLower().contains(filterText);
    bool matchesCategory =
        filterCategory.isEmpty() ||
        (orderMenuTable->item(r, 2)->text() == filterCategory);
...
void MainWindow::onAddItemClicked() {
  ...
  if (selectedRow < 0) {
    QMessageBox::warning(
        this, "No Selection",
        "Please select a menu item from the select menu list.");
    return;
  }

  std::string itemID =
      orderMenuTable->item(selectedRow, 0)->text().toStdString();
  ...
  if (qty <= 0 || qty > 100) {
    QMessageBox::warning(this, "Quantity Error",
...
void MainWindow::onRemoveItemClicked() {
  ...
  if (selectedRow < 0) {
    QMessageBox::warning(
        this, "No Selection",
        "Please select an item from the order list to remove.");
    return;
  }
...
void MainWindow::updateOrderTable() {
  ...
  for (size_t i = 0; i < items.size(); ++i) {
    ...
    orderTable->setItem(
        i, 0, new QTableWidgetItem(QString::fromStdString(item->getName())));
    orderTable->setItem(
        i, 1,
        new QTableWidgetItem(QString("$%1").arg(item->getPrice(), 0, 'f', 2)));
  ...
  orderSubtotalLabel->setText(
      QString("Subtotal: $%1").arg(tempSubtotal, 0, 'f', 2));
}
...
void MainWindow::onCalculateBillClicked() {
  ...
  if (selectedIdx < 0) {
    QMessageBox::warning(this, "No Active Order",
                         "Please select an active table order to bill.");
    return;
  }
  ...
  std::string orderDate = QDateTime::currentDateTime()
  ...
  double subtotal =
      dineInOrder.computeTotal(); // Includes base prices + prep fees
  ...
  totalLabel->setText(
      QString("Grand Total: $%1").arg(bill.getTotal(), 0, 'f', 2));

  // Feature 3: Show red warning label if Bill exceeds $500
  if (bill.getTotal() > 500.0) {
    largeBillWarningLabel->setText(
        QString("⚠️ Large Bill Warning: Total exceeds $500.00! ($%1)")
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (!hasAlpha) {
      QMessageBox::warning(
          ...
          "Item name must contain at least one alphabetic character.");
      return;
    }
    ...
    if (price <= 0.0 || price > 10000.0) {
      QMessageBox::warning(
          this, "Validation Error",
          "Price must be greater than zero and under $10,000.");
      return;
    }
    ...
    for (const auto &item : menuItems) {
      if (item && item->getItemID() == newID) {
        duplicate = true;
        break;
      }
    }
    if (duplicate) {
      QMessageBox::critical(this, "Duplicate Item ID",
    ...
    if (categoryType == "FOOD") {
      ...
      newItem = std::make_shared<BeverageItem>(
          newID, name.toStdString(), "Beverage", price, dialog.getVolume(),
          dialog.getBeverageType().toStdString(), dialog.getIsCarbonated());
    }
...
void MainWindow::onMenuItemChanged(QTableWidgetItem *item) {
  ...
  if (ok && newPrice > 0.0 && newPrice <= 10000.0) {
    ...
    QMessageBox::warning(
        this, "Invalid Price",
        "Price must be a valid positive number under $10,000.");
  }
...
void MainWindow::onPaymentBillOrMethodChanged() {
  int index = paymentsBillCombo->currentIndex();
  if (index < 0) {
    paymentAmountSpinner->setValue(0.0);
    paymentAmountSpinner->setEnabled(false);
    return;
  }

  std::string billID =
      paymentsBillCombo->itemData(index).toString().toStdString();
  double totalAmount = 0.0;
  for (const auto &b : restaurantBills) {
  ...
  if (method == "Card") {
    paymentAmountSpinner->setValue(totalAmount);
    paymentAmountSpinner->setEnabled(false);
  } else {
    paymentAmountSpinner->setValue(totalAmount);
    paymentAmountSpinner->setEnabled(true);
  }
}
...
void MainWindow::onProcessPaymentClicked() {
  ...
  // If card, the payment amount is the exact total
  if (method == "Card") {
    paymentAmount = totalAmount;
  }

  // Validate payment amount (must be at least totalAmount for Cash)
  if (method == "Cash" && paymentAmount < totalAmount - 0.005) {
    QMessageBox::warning(
        this, "Insufficient Amount",
        QString("Payment amount ($%1) is less than the bill total ($%2).")
  ...
  if (method == "Cash") {
    ...
    reply = QMessageBox::question(
        this, "Confirm Payment",
        QString("Bill Total: $%1\nDo you want to process this payment (Card)?")
            .arg(totalAmount, 0, 'f', 2),
        QMessageBox::Yes | QMessageBox::No);
  }
  ...
  std::string paymentID =
      "PAY-" + std::to_string(restaurantPayments.size() + 1001);

  Payment payment(paymentID, billID, method, totalAmount, paymentAmount);
  if (payment.processPayment()) {
    ...
    for (int i = 1; i <= 8; ++i) {
      ...
      if (table && table->hasActiveOrder() &&
          table->getCurrentOrder()->getOrderID() ==
              selectedBill->getOrderID()) {
        table->setOccupied(false);
        break;
      }
    }
...
void MainWindow::onSaveDataTriggered() {
  ...
  if (DataManager::saveData("data", dineInOrders, restaurantBills,
                            restaurantPayments, &err)) {
    QMessageBox::information(this, "Saved",
                             "Transaction records saved successfully!");
...
void MainWindow::onLoadDataTriggered() {
  std::string err;
  if (DataManager::loadData("data", dineInOrders, restaurantBills,
                            restaurantPayments, &err)) {
    unsavedChanges = false;
    refreshBillingOrderCombo();
    refreshPaymentsBillCombo();
    ...
    QMessageBox::information(this, "Loaded",
                             "Transaction records loaded successfully!");
  } else {
...
void MainWindow::refreshBillingOrderCombo() {
  ...
  for (int i = 1; i <= 8; ++i) {
    auto table = manager.getTable(i);
    if (table && table->hasActiveOrder() &&
        !table->getCurrentOrder()->isEmpty()) {
      billingOrderCombo->addItem(
          QString("Table %1 - %2")
              .arg(i)
...
void MainWindow::updateAnalyticsDisplay() {
  ...
  int cancelledCount =
      orderEngine.computeOrdersByStatus(dineInOrders, "Cancelled");

  dailySalesLabel->setText(
      QString("Daily Sales Total: $%1").arg(totalSales, 0, 'f', 2));
  avgBillLabel->setText(
      QString("Average Bill Amount: $%1").arg(avgBill, 0, 'f', 2));

  ordersOpenLabel->setText(QString("Open Orders: %1").arg(openCount));
  ordersPaidLabel->setText(QString("Paid Orders: %1").arg(paidCount));
  ordersCancelledLabel->setText(
      QString("Cancelled Orders: %1").arg(cancelledCount));

  popularItemsList->clear();
  auto popularity = orderEngine.computePopularItems(dineInOrders);
  std::vector<std::pair<std::string, int>> popularVec(popularity.begin(),
                                                      popularity.end());
  std::sort(popularVec.begin(), popularVec.end(),
...
void MainWindow::onExportMenuTriggered() {
  QString fileName =
      QFileDialog::getSaveFileName(this, "Export Menu File", "menu_export.txt",
...
void MainWindow::onExportReceiptTriggered() {
  ...
  if (receiptText.isEmpty()) {
    QMessageBox::warning(this, "Empty Receipt",
                         "There is no receipt to export.");
    return;
  }

  QString fileName =
      QFileDialog::getSaveFileName(this, "Export Receipt File", "receipt.txt",
                                   "Text Files (*.txt);;All Files (*)");
  if (fileName.isEmpty())
    return;
...
void MainWindow::onPlusClicked() {
  ...
  if (val < 100)
    quantitySpinner->setValue(val + 1);
}
...
void MainWindow::applyTheme(bool dark) {
  ...
  if (dark) {
    this->setStyleSheet(
        ...
        "font-size: 14px; }"
        "QMenuBar { background-color: #1e293b; color: #f8fafc; border-bottom: "
        "1px solid #334155; }"
        "QMenuBar::item { background-color: transparent; padding: 6px 12px; }"
        "QMenuBar::item:selected { background-color: #334155; color: #60a5fa; "
        "border-radius: 4px; }"
        "QMenu { background-color: #1e293b; color: #f8fafc; border: 1px solid "
        "#334155; border-radius: 6px; padding: 4px; }"
        "QMenu::item:selected { background-color: #334155; color: #60a5fa; "
        "border-radius: 4px; }"
        "QTabWidget::pane { border: 1px solid #334155; border-radius: 12px; "
        "background-color: #1e293b; padding: 10px; }"
        "QTabBar::tab { background-color: #1e293b; color: #94a3b8; border: 1px "
        "solid #334155; border-bottom: none; border-top-left-radius: 6px; "
        "border-top-right-radius: 6px; padding: 8px 16px; margin-right: 2px; }"
        "QTabBar::tab:selected { background-color: #2563eb; color: white; "
        "border-color: #2563eb; }"
        "QTableWidget { background-color: #1e293b; border: 1px solid #334155; "
        "border-radius: 12px; gridline-color: #334155; "
        ...
        "border-radius: 12px; color: #cbd5e1; font-family: 'Courier New', "
        "monospace; font-size: 13px; padding: 8px; }"
        "QPushButton { background-color: #2563eb; color: white; border: none; "
        "padding: 9px 18px; border-radius: 6px; font-weight: bold; }"
        "QPushButton:hover { background-color: #3b82f6; }"
        "QPushButton:pressed { background-color: #1d4ed8; }"
        "QPushButton#spinnerBtn { background-color: #1e293b; border: 1px solid "
        "#334155; color: #60a5fa; font-size: 8px; padding: 0px; border-radius: "
        "2px; }"
        "QPushButton#spinnerBtn:hover { background-color: #334155; }"
        "QPushButton#spinnerBtn:pressed { background-color: #2563eb; color: "
        "white; }"
        "QComboBox { background-color: #1e293b; border: 1px solid #334155; "
        "border-radius: 8px; padding: 6px 12px; color: #f8fafc; min-height: "
        "20px; }"
        "QComboBox::drop-down { subcontrol-origin: padding; "
        "subcontrol-position: top right; width: 25px; border: none; }"
        "QComboBox::down-arrow { border-left: 5px solid transparent; "
        "border-right: 5px solid transparent; border-top: 5px solid #60a5fa; "
        ...
        "border-radius: 12px; color: #cbd5e1; padding: 8px; }"
        "QLabel { font-weight: 500; }"
        "QWidget#headerWidget { background-color: #1e293b; border: 1px solid "
        "#334155; border-left: 5px solid #60a5fa; border-radius: 12px; }"
        "QLabel#titleLabel { font-size: 22px; font-weight: bold; color: "
        "#60a5fa; }"
    ...
    this->setStyleSheet(
        ...
        "font-size: 14px; }"
        "QMenuBar { background-color: #ffffff; color: #0f172a; border-bottom: "
        "1px solid #e2e8f0; }"
        "QMenuBar::item { background-color: transparent; padding: 6px 12px; }"
        "QMenuBar::item:selected { background-color: #f1f5f9; color: #2563eb; "
        "border-radius: 4px; }"
        "QMenu { background-color: #ffffff; color: #0f172a; border: 1px solid "
        "#e2e8f0; border-radius: 6px; padding: 4px; }"
        ...
        "QTabBar::tab { background-color: #f1f5f9; color: #64748b; border: 1px "
        "solid #e2e8f0; border-bottom: none; border-top-left-radius: 6px; "
        "border-top-right-radius: 6px; padding: 8px 16px; margin-right: 2px; }"
        "QTabBar::tab:selected { background-color: #2563eb; color: white; "
        "border-color: #2563eb; }"
        ...
        "border-radius: 12px; color: #334155; font-family: 'Courier New', "
        "monospace; font-size: 13px; padding: 8px; }"
        "QPushButton { background-color: #2563eb; color: white; border: none; "
        "padding: 9px 18px; border-radius: 6px; font-weight: bold; }"
        "QPushButton:hover { background-color: #3b82f6; }"
        "QPushButton:pressed { background-color: #1d4ed8; }"
        "QPushButton#spinnerBtn { background-color: #ffffff; border: 1px solid "
        "#e2e8f0; color: #2563eb; font-size: 8px; padding: 0px; border-radius: "
        "2px; }"
        "QPushButton#spinnerBtn:hover { background-color: #f1f5f9; }"
        "QPushButton#spinnerBtn:pressed { background-color: #1d4ed8; color: "
        "white; }"
        "QComboBox { background-color: #ffffff; border: 1px solid #e2e8f0; "
        "border-radius: 8px; padding: 6px 12px; color: #0f172a; min-height: "
        "20px; }"
        "QComboBox::drop-down { subcontrol-origin: padding; "
        "subcontrol-position: top right; width: 25px; border: none; }"
        "QComboBox::down-arrow { border-left: 5px solid transparent; "
        "border-right: 5px solid transparent; border-top: 5px solid #2563eb; "
        "width: 0; height: 0; margin-right: 8px; }"
        ...
        "border-radius: 12px; color: #334155; padding: 8px; }"
        "QLabel { font-weight: 500; }"
        "QWidget#headerWidget { background-color: #ffffff; border: 1px solid "
        "#e2e8f0; border-left: 5px solid #2563eb; border-radius: 12px; }"
        "QLabel#titleLabel { font-size: 22px; font-weight: bold; color: "
        "#2563eb; }"
